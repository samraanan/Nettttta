...
export function ScheduleEditor({ onClose }) {
    ...
    const handleResetScore = async () => {
        if (!resetConfirm) {
            ...
            // Reset the state after 3 seconds if not confirmed
            setTimeout(() => setResetConfirm(false), 3000);
            return;
        }

        const dateStr = prompt('הכנס תאריך להתחלת צבירת הנקודות (YYYY-MM-DD):', format(new Date(), 'yyyy-MM-dd'));
        if (dateStr) {
            await storageService.recalculateScore(dateStr);
            alert("הניקוד אופס והחישוב מחדש בוצע בהצלחה.");
        }
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex justify-between items-center sticky top-4 z-10">
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className={clsx(
                                    "text-xs px-3 py-2 rounded transition font-medium",
                                    resetConfirm
                                        ? "bg-red-600 text-white hover:bg-red-700 animate-pulse"
                                        : "text-red-500 hover:text-red-700 hover:bg-red-50"
                                )}
                                title="אפס ניקוד מערכת"
                            >
                                {resetConfirm ? "לחץ שוב לאישור" : "אפס נקודות..."}
                            </button>
                            <button
